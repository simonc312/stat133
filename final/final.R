...
name <- "Shiwei Chen"
github.name <- "simonc312"
email.address <- "simon.c312@gmail.com"
...
# with y[i]=T if x[i] is divisible by 10, otherwise F
  ...
  y <- sapply(x, function(i){ return(i%% 10 == 0)})
...
# Each vector of length 100 of Poisson (hint:rpois) random variables with mean 5
  set.seed(71)
l <- lapply(as.list(1:12),function(e){rpois(100,5)})
...
f1 <- family[which(family$gender == "f" & family$age >= 50),]

  
# [1 pt]
# Create [f2] a subset of family with only men 6 foot tall or more
f2 <- family[which(family$gender == "m" & family$height >= 72),]
...
married_first_time_mothers <- infants[which(infants$marital == "Married" & infants$parity == 1),]
t <- table(married_first_time_mothers$ed)


# [2 pt]
# Calculate [mw], the average birthweight ($bwt) of all babies whose were full term, i.e. gestation equal or more than 259 days.

mw <- mean(infants$bwt[which(infants$gestation >= 259)])
...
cols1 <- rainbow(3)
cols <- cols1[as.numeric(iris$Species)]
plot(y=iris$Petal.Width,x=iris$Petal.Length,xlab="Petal Length",ylab="Petal Width",col=cols)
legend('topleft',levels(iris$Species),col=cols1,pch=1)
# [3 pt]
# Make a scatterplot of ( sepal length / petal length) as a function of index (order)
# Color the plotting symbol by Species (any 3 colors)
plot(y=iris$Sepal.Length/iris$Petal.Length,x=1:length(iris$Sepal.Length),xlab="index",ylab="Sepal Length / Petal Length",col=cols)
legend('topright',levels(iris$Species),col=cols1,pch=1)
...
plot(x=infants$gestation,y=infants$bwt,xlab="gestation in days",ylab="birthweight in ounces",pch="*",type="p",col=c("red"))
abline(v=259)
text(x=259,y=60,labels=c("259"))
# [6 pts]
...
# Add x-axis labels
hist(infants$age[which(!is.na(infants$age))],ylab="Number of Mother's of Age X",xlab="Mother's Age",main="Histogram of Mother's Age")
points(table(infants$age),col = c('blue'),type="l")
#################################################################
...
mean.long.cache <-  sapply(Cache500,function(e){
  if(length(e)>=50)
    mean(e,na.rm=TRUE)
  else
    return(NA)
...
max.petal.width <- by(iris[,c("Petal.Width")],iris[,c("Species")],function(col){max(col)})
...
longerRange <- function(m1, m2){
  r1 = range(m1,na.rm=TRUE) #range returns a min and a max ignoring NA values
  
  r2 = range(m2,na.rm=TRUE)
  if(abs(r1[1] - r1[2]) >= abs(r2[1] - r2[2]))
    return(1)
  else
    return(2)
}
...
dice_sum <- function(k, B){
  sapply(1:B,function(iteration){return(sum(sample(1:6,k)))})
}
...
results <- sapply(Bvec,function(b){dice_sum(2,b)})
ave.diceRoll <- sapply(results,function(test){mean(test)})
sd.diceRoll <-  sapply(results,function(test){sd(test)})
hist(results[[1]],xlab="sum of dice roll",main="Histogram for B=20")
hist(results[[2]],xlab="sum of dice roll",main="Histogram for B=100")
hist(results[[3]],xlab="sum of dice roll",main="Histogram for B=1000")
hist(results[[4]],xlab="sum of dice roll",main="Histogram for B=5000")
...
tests <- sapply(1:200,function(i){paste0("test",i)})
...
tests.all <- paste0(tests,collapse=' ')
...
minchin.split <- unlist(strsplit(toupper(minchin)," "))
