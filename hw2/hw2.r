...
# bmi * ((2.54/100 * height)^2) * 2.2 = weight  
# Now calculate OW_weight 
ow_weight <-  ((0.0254 * family$height)^2) * 2.2 * ow_limit
...
plot( ow_weight ~ family$weight,
      main= "A family matter",
      xlab="actual weight",
      ylab="hypothetical overweight")
abline(0,1)
...
BerkArea <- BerkArea[(BerkArea$price < quantile(BerkArea$price,.99, na.rm = TRUE)) & (BerkArea$bsqft < quantile(BerkArea$bsqft,.99, na.rm = TRUE)),] 
...
brfunction <- function(element){
  if(is.na(element) | element <= 5){return(element)}
  else{return(5)}
}
br5 <- sapply(BerkArea$br,brfunction)
...
#The older houses have more bedrooms on average and there were many
#one bedroom condos or apts built in 1980s.
