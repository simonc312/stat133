...
# Hicham El Guerrouj 1998 most recent year in data frame 
wr.name <- wr1500m$athlete[length(wr1500m$athlete)]
...
# year is discrete numeric and ordinal. time is continuous numeric and ordinal.
...
plot(wr1500m$year,wr1500m$times_sec, type='s',xlab='year',ylab='time in seconds')
...
# your code here
wr1500m$month[is.na(wr1500m$month)] <- 0.5
new_year <- wr1500m$year + wr1500m$month/12
wr1500m$new_year <- new_year
plot(wr1500m$new_year,wr1500m$times_sec, type='s',xlab='year and month',ylab='time in seconds')
...
plot(wr1500m$new_year,wr1500m$times_sec, type='s',xlim = c(1892,2014), xlab='year and month',ylab='time in seconds')
lines( c(as.numeric(wr1500m$new_year[51]),2014),c(wr_1998,wr_1998) )
...
wr.name2 <- wr1500m$athlete[wr1500m$year == 1944]
abline(v = c(wr1500m$new_year[wr1500m$year==1944],wr_1944),col="grey60")
abline(v = c(wr1500m$new_year[wr1500m$year==1998],wr_1998),col="grey60")

text(wr1500m$new_year[wr1500m$year==1944],wr_1944,wr.name2,font=9,srt = 90, adj=c(0,1))
text(wr1500m$new_year[wr1500m$year==1998],wr_1998,wr.name,font=9,srt = 90, adj=c(0,1))
...
plot(wr1500m$new_year,wr1500m$times_sec, type='s',xlim = c(1892,2014), xlab='year and month',ylab='time in seconds',main="Men's 1500 Meter World Records")
lines( c(as.numeric(wr1500m$new_year[51]),2014),c(wr_1998,wr_1998) )
abline(v = c(wr1500m$new_year[wr1500m$year==1944],wr_1944),col="grey60")
abline(v = c(wr1500m$new_year[wr1500m$year==1998],wr_1998),col="grey60")
...
### population : discrete integer

# What about Total?
### Total : discrete integer
...
# Values are plotted on top of each other
#Poor scale used so there is lots of empty space and bulk of information is obscured
# Information poor as we cannot tell which countries the outliers are 
...
symbols(log(SO2012Ctry$GDP_per_person),
        log(SO2012Ctry$pop), 
        circles = SO2012Ctry$Total )
...
symbols(log(SO2012Ctry$GDP_per_person[SO2012Ctry$Total>0]),
        log(SO2012Ctry$pop[SO2012Ctry$Total>0]),
        circles = SO2012Ctry$Total[SO2012Ctry$Total>0])
points(log(SO2012Ctry$GDP_per_person[SO2012Ctry$Total==0]),
        log(SO2012Ctry$pop[SO2012Ctry$Total==0]),
        pch='.',col="red")
...
top5 <- order((SO2012Ctry$Total > 40),decreasing=TRUE)[1:5]
# your plotting code here, including a new call to text() 
rCols <- rainbow(5, alpha = 0.25)
rCols[6] <- "grey60"
colorMapFunction <- function(element){
  if(element %in% SO2012Ctry$Country[top5])
    {return(rCols[which(SO2012Ctry$Country[top5] == element)])}
  else{return(rCols[6])}
}
color_vector <- sapply(SO2012Ctry$Country[SO2012Ctry$Total>0],colorMapFunction)
symbols(log(SO2012Ctry$GDP_per_person[SO2012Ctry$Total>0]),
        log(SO2012Ctry$pop[SO2012Ctry$Total>0]),
        circles = SO2012Ctry$Total[SO2012Ctry$Total>0],
        xlab="Log Scaled GDP per person",
        ylab="Log Scaled Population",
        main="2012 Summer Olympics",
        sub="Relationship between Medal Count, GDPPP, & Population size",
        bg= color_vector)
points(log(SO2012Ctry$GDP_per_person[SO2012Ctry$Total==0]),
       log(SO2012Ctry$pop[SO2012Ctry$Total==0]),
       pch='.',col="red")
legend(legend = SO2012Ctry$Country[top5], fill = rCols, "bottomleft")
text(log(SO2012Ctry$GDP_per_person[top5]),
     log(SO2012Ctry$pop[top5]),
     SO2012Ctry$Country[top5],col="red"
     )
...
wonMedal <- SO2012Ctry[SO2012Ctry$Total>0,c("latitude","longitude","Country","Total")]
world <- map(database="world")
symbols(wonMedal$longitude,wonMedal$latitude,circles=wonMedal$Total*0.3,add=TRUE)
...
world <- map(database="world")
symbols(wonMedal$longitude,wonMedal$latitude,circles=wonMedal$Total*0.3,bg=myGold,add=TRUE)
...
par(las=2, cex=0.8)
barplot(athTab,beside=TRUE,col=c("Blue","Red"),main="#Athletes by Sex & Sport in London 2012 Olympics")
legend(legend = c("Female","Male"), fill = c("Blue","Red"), "topright")
# You are DONE.
